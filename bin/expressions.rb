  ...
  def num_symbols
    ...
    elements.reverse_each do |element|
      if element.list?
        # recurse into expressions in list
        sublist = element.list
        vars += AbstractExpression.parsed_expressions_num_symbols(sublist)
      elsif element.numeric_constant? && element.symbol
        vars << element
      end
    end
  ...
  def text_symbols
    ...
    @elements.each do |element|
      if element.list?
        # recurse into expressions in list
        sublist = element.list
        strs += AbstractExpression.parsed_expressions_text_symbols(sublist)
      elsif element.text_constant? && element.symbol
        strs << element
      end
    end
...
class AbstractExpression
  ...
  def num_symbols
    AbstractExpression.parsed_expressions_num_symbols(@expressions)
  end
  ...
  def text_symbols
    AbstractExpression.parsed_expressions_text_symbols(@expressions)
  end
