...
class AbstractForControl < AbstractLoopControl
  ...
  def bump_control(interpreter)
    ...
    interpreter.unlock_variable(@variable_name) if
      $options['lock_fornext'].value

    interpreter.set_value(@variable_name, current_value)

    interpreter.lock_variable(@variable_name) if
      $options['lock_fornext'].value
  end
end
...
class Interpreter
  ...
  def get_record_read_variables(line_number)
    raise BASICRuntimeError.new(:te_rec_no_read, line_number.to_s) unless
  ...
  def get_record_write_variables(line_number)
    raise BASICRuntimeError.new(:te_rec_no_write, line_number.to_s) unless
