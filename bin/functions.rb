...
class FunctionAsc < AbstractScalarFunction
  ...
  def evaluate(interpreter, stack)
    ...
    if match_args_to_signature(args, @signature)
      ...
      raise BASICRuntimeError.new(:te_str_empty) if
        text.empty?
...
class FunctionInstr < AbstractScalarFunction
  ...
  def evaluate(_, stack)
    ...
    raise BASICRuntimeError.new(:te_val_out, "Invalid start index for #{@name}()") if
      start < 1
...
class FunctionLeft < AbstractScalarFunction
  ...
  def evaluate(_, stack)
    ...
    raise BASICRuntimeError.new(:te_count_inv, "Invalid count for #{@name}()") if
      count < 0
...
class FunctionMid < AbstractScalarFunction
  ...
  def evaluate(_, stack)
    ...
    raise BASICRuntimeError.new(:te_len_inv, "Invalid length for #{@name}()") if
      length < 0
