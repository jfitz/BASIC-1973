...
class BinaryOperator < AbstractElement
  ...
  def set_arguments(stack)
    raise(BASICExpressionError, 'Not enough operators') if stack.size < 2
...
class BinaryOperatorMax < BinaryOperator
  ...
  def set_content_type(stack)
    ...
    raise(BASICExpressionError, "Type mismatch #{a_type} #{@op} #{b_type}") if
      !arg_types.include?(a_type) || !arg_types.include?(b_type) || b_type != a_type
  ...
  def matrix_matrix(x, y)
    op_matrix_matrix(:maximum, x, y)
  end

  def matrix_scalar(x, y)
    op_matrix_scalar(:maximum, x, y)
  end

  def scalar_matrix(x, y)
    op_scalar_matrix(:maximum, x, y)
  end

  def array_array(x, y, base)
    op_array_array(:maximum, x, y, base)
  end

  def array_scalar(x, y, base)
    op_array_scalar(:maximum, x, y, base)
  end

  def scalar_array(x, y, base)
    op_scalar_array(:maximum, x, y, base)
  end

  def op_scalar_scalar(x, y)
    x.public_send(:maximum, y)
  end
end
...
class BinaryOperatorMin < BinaryOperator
  ...
  def matrix_matrix(x, y)
    op_matrix_matrix(:minimum, x, y)
  end

  def matrix_scalar(x, y)
    op_matrix_scalar(:minimum, x, y)
  end

  def scalar_matrix(x, y)
    op_scalar_matrix(:minimum, x, y)
  end

  def array_array(x, y, base)
    op_array_array(:minimum, x, y, base)
  end

  def array_scalar(x, y, base)
    op_array_scalar(:minimum, x, y, base)
  end

  def scalar_array(x, y, base)
    op_scalar_array(:minimum, x, y, base)
  end

  def op_scalar_scalar(x, y)
    x.public_send(:minimum, y)
  end
end
