...
My reference manual has been Kemeny and Kurtz's "BASIC Programming" (John Wiley and Sons, 1967) and David Ahl's "101 BASIC Computer Games" (DEC, 1973).
...
Commands ("RUN", "LIST"), statements ("PRINT", "LET"), function names, and variable names must be in upper case. Lower case causes an error.
...
- The IF/THEN statement has variations of IF expression statement, IF expression THEN statement, IF expression THEN statement ELSE statement, and IF expression THEN multiple statements, all of which are controlled by the expression
...
|--ignore-randomize |Forces the compiler to ignore the RANDMIZE statement. Successive runs of the interpreter will use the same sequence of numbers from the RND() function. (Useful for testing.)
...
Immediate commands are executed on the command line. They are not stored as part of the program. They have no line number. They are available only in "run" mode.
...
Loads a stored program into memory. Sorts statements by line number.
...
Variables store numeric and text values. Numeric variables handle integer and floating point values automatically.
...
Multiple statements may be placed on a single line, separated by backslash characters.
...
(Without a DIM statement, variables may have a dimension of at most 10.)
...
BASIC-1973 checks for neither, and will allow an END in the middle of the program and will allow multiple END statements.
...
File names must be enclosed in double-quotres.
...
This prompt can be changed to any text value by specifying the value as the first parameter.
...
The –ignore-randomize option disables the RANDOMIZE statement (the interpreter allows it but ignores it).
...
You may READ in pairs and define ten values per DATA statement. You may READ ten items from DATA statements that contain one value each.
...
|RND() |numeric |Equivalent to RND(100).
|EXP(x) |numeric |Computes ex.
...
|TRN(m) |numeric |Transposes values in a matrix, exchanging rows and columns.
|ZER(x) |numeric |Creates an array containing all zeros, with x columns
|ZER(x,y) |numeric |Creates a matrix containing all zeros, with x rows and y columns.
|CON(x) |numeric |Creates an array containing all ones, with x columns
|CON(x,y) |numeric |Creates a matrix containing all ones, with x rows and y columns.
|IDN(x) |numeric |Creates an identity matrix (all zeros except for ones on the diagonal).
|IDN(x,x) |numeric |Same as IDN(x). The two values must be identical, as identity matrices are always square.
...
|UNPACK(t)|array |Converts a string variable (or expression) to an array of numeric values. The first element (index 0) contains the number of converted elements. Each element is the ASCII value for the corresponding character in the original string.
...
- Option for multistatement lines to be separated by colon (or colon and backslash)
- Functions for strings (MID$(), VAL(), LEFT$(), RIGHT$())
- Option to use subscripts to string variables to extract substring
- Allow statement keywords to be abbreviated to three letters
- LINE INPUT to read a single string variable
- LINPUT abbreviation of LINE INPUT
- PRINT keyword can be replaced with an ampersand
- REM keyword can be replaced with an apostrophe
- AND and OR in expressions of IF statements
- RANDOM statement (same as RANDOMIZE)
- ON expression GOTO line numbers
- ON expression GOSUB line numbers
- CHAIN filename
- Statement modifiers (IF, UNLESS, WHILE, UNTIL, FOR) at the end of a statement
