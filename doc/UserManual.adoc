...
|--precision |Sets the precision for numeric variables. Default is 6, maximum is 15.
...
|BRACKETS |TRUE or FALSE |Allow square brackets to denote subscripts
...
|MATCH_FORNEXT |TRUE or FALSE |FALSE |Require exact pairing of FOR and NEXT statements
|MIN_MAX_OP |TRUE or FALSE |FALSE |Allow MIN and MAX operators
|NEWLINE_SPEED |numeric |0 |Limit printing speed only for newlines
|PRECISION |numeric 1 to 15 |5 |Sets the precision for numeric values
...
The invokation of the function may provide a variable name or a value.
(See examples below.)
...
You cannot GOTO or GOSUB from the main program into the body of a user-defined function.
If you do, lines in the function are ignored until after the FNEND statement.
You cannot GOTO or GOSUC out of the body of a user-defined function to the main program or from one user-defined function to another user-defined function.
Such a transfer will cause an error.
...
CHANGE, CLOSE, GOSUB, GOTO, INPUT, INPUT$, LET, (LET-less assignment), LINE INPUT, OPEN, PRINT, PRINT USING, RANDOMIZE, READ, RESTORE, RETURN, SLEEP, STOP, TRACE, WRITE, ARR, ARR PRINT, ARR READ, ARR WRITE, MAT, MAT PRINT, MAT READ, MAT WRITE
